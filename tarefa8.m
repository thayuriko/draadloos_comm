...
grid on; axis([1 d(end) min(P_mrc(4,:)) 1]);
...
plot = figure('units','normalized','outerposition',[0 0 1 1]);
subplot(1,2,1)
...
semilogy(SNR_avg_db, P_mrc(4,:),'-','LineWidth',2); hold off;
...
subplot(1,2,2)
semilogy(SNR_avg_db, P_sc(1,:),'-','LineWidth',2); hold on;
...
grid on; axis([SNR_avg_db(1) SNR_avg_db(end) 1e-15 1]);
title('Probabilidade de Outage no canal Rayleigh com SC');
legend('nRx = 1','nRx = 2','nRx = 4','nRx = 8');
ylabel('P_{out}'); xlabel('SNR (dB)');

print(plot,'t8_q2','-dpng');

%% Q3. Como determinar a P_out completa considerando que o relay pode não decodificar a msg?
...
for j=1:length(nRx)
    for i=1:length(SNR_avg_db)
        ...
        P_rayl(j,i) = gammainc(SNR_min*nRx(j)/SNR_avg(i),nRx(j),'lower')/gamma(nRx(j));
        P_coop(j,i) = (1-exp(-SNR_min/SNR_avg(i)))^nRx(j);
    end
end